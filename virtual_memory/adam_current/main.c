		  ...
		  u64       num_pages)
{
    ...
    list_for_each_entry(tmp_pool, &petmem_pool_list, node) {
        //printk("list for each\n tmp_pool->base_addr: %p\ntmp_pool->base_addr +(0x1 << tmp_pool->pool_order): %p\n", tmp_pool->base_addr, tmp_pool->base_addr + (0x1 << tmp_pool->pool_order));
	//printk("page order: %d\n", page_order);	

	if ((page_addr >= tmp_pool->base_addr) && 
           (page_addr <  tmp_pool->base_addr + (0x1 << tmp_pool->pool_order))) {
           
           printk("__va(page_addr): %p\n", __va(page_addr));
