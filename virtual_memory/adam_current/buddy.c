...
{
//        printk("block: %ul\nmp->base_addr: %ul\n", block, mp->base_addr);
//     	  printk("block - mp->base_addr: %ul\n", (unsigned long)block - mp->base_addr);
//        printk("mp->min_order: %d\n", mp->min_order);
//	  printk("^^ >> mp->min_order: %ul\n", ((unsigned long)block - mp->base_addr) >> mp->min_order);
	
	unsigned long block_id =
		((unsigned long)block - mp->base_addr) >> mp->min_order;
        	
	//printk("block_id: %ul    mp->num_blocks: %ul\n", block_id, mp->num_blocks);
