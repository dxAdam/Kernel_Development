	   ...
	   int              recurse)
{
    ...
    /* DO NOT REFERENCE THESE VARIABLES */
    /* They only exists to provide version compatibility */
...
#else
    inode_unlock(path_ptr->dentry->d_inode);
#endif

    path_put(path_ptr);
#else
    if (!IS_ERR(dentry)) {
	ret = vfs_mkdir(d_inode(path_ptr->dentry), dentry, perms);
    }

    done_path_create(path_ptr, dentry);
...
int
file_close(struct file * file_ptr)
{
...
#else 
    ret = vfs_getattr(&(file_ptr->f_path), &s, STATX_BASIC_STATS, AT_STATX_SYNC_AS_STAT);
#endif
